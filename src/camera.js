...
export class CameraOrbit {
  ...
  initCam2D() {
    let aspect =
      this.params.container.clientWidth / this.params.container.clientHeight;
    let d = 5;
  ...
  startCam3D(params) {
    ...
    if (button == "left") {
      ...
      let dir = new THREE.Vector3()
        .subVectors(
          camera3D.userData.camera.d3.targetO.position,
          camera3D.position
        )
        .normalize();
      ...
      let dergree =
        THREE.Math.radToDeg(dir.angleTo(new THREE.Vector3(dir.x, 0, dir.z))) *
        2;
      if (dir.y > 0) {
        dergree *= -1;
      }
      ...
      camera3D.userData.camera.d3.theta =
        THREE.Math.radToDeg(Math.atan2(dir.x, dir.z) - Math.PI) * 2;
  ...
  moveCam3D(camera3D, event, click) {
    ...
    if (click == "left") {
      let radious = camera3D.userData.camera.d3.targetO.position.distanceTo(
        camera3D.position
      );

      let theta =
        -((event.clientX - this.mouse.pos.x) * 0.5) +
        camera3D.userData.camera.d3.theta;
      let phi =
        (event.clientY - this.mouse.pos.y) * 0.5 +
        camera3D.userData.camera.d3.phi;
      ...
      camera3D.userData.camera.d3.targetO.rotation.set(
        0,
        camera3D.rotation.y,
        0
      );
    ...
    if (click == "right") {
      ...
      let offset = new THREE.Vector3().subVectors(
        camera3D.userData.camera.click.pos,
        intersects[0].point
      );
  ...
  rayIntersect(event, obj, t) {
    ...
    function getMousePosition(event) {
      let x =
        ...
        1;
      let y =
        -((event.clientY - container.offsetTop) / container.clientHeight) * 2 +
        1;
